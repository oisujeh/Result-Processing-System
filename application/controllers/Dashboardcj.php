...
//            $this->check_vehicle_condition();
    ...
    public function register_course()
    {
        ...
        $get_trans = $this->db->query("select * from tblcourses where level= $s_level order by id");
        ...
        if (isset($_POST['add'])) {
            ...
            foreach ($checkbox as $user_selected) {
                ...
                if ($check_course->num_rows() <= 0) {
                    ...
                    $get_co = $this->db->query("select sum(tblcourses.load) as cload from  tblcourse_reg inner join tblcourses on tblcourses.id = tblcourse_reg.course_id
 where mat_no ='$matno' and tblcourse_reg.level_id = $level_id and tblcourse_reg.session_id = $session_id");

                    if($get_co->row()->cload >= 50){
                        $this->session->set_flashdata('msg', '<div class="alert alert-info text-center"> Course Registration should not exceed 50 credits </div>');
                    ...
                    $this->session->set_flashdata ( 'msg', '<div class="alert alert-danger text-center"> Already registered courses were skipped! </div>' );
                 //   redirect ( base_url ( 'dashboard/register_course/'.$student_id) );
                }
