       ...
       //});
        ...
        $("#amount_buying").on("input",function (e) {
            ...
            $.getJSON('/memberarea/loadamount?ecurrencyname=' + ecurrencyname, function (json) {
                ...
                label2.val(json.we_sell * amount);
            });
        ...
        $("#amount").on("input",function (e) {
            var amount = this.value;
            var currency = $("#tcurrency").val();
            var label2 = $('#loading');
            var price_in_btc = $("#price_in_btc");
            var price_in_usd = $("#price_in_usd");

            $.getJSON('/home/get_exchange_rate?from=' + currency + '&amount=' + amount, function (json) {
                //alert(json.we_buy * amount);
                label2.addClass("text-success");
                price_in_btc.val(((0.15 * json.price) + json.price).toFixed(4));
                label2.html("<b>You will pay " + ((0.15 * json.price) + json.price).toFixed(4) + " BTC </b>");
                var amountinbtc = ((0.15 * json.price) + json.price).toFixed(4);
                $.getJSON('https://apiv2.bitcoinaverage.com/convert/global?from=BTC&to=USD&amount=' + amountinbtc, function (json2) {
                    price_in_usd.val(json2.price);
                });
            });
        ...
        $("#amountbtc").on("input",function (e) {
            var type = $("#type").val();
            //var selectecurrency = $("#selectecurrency").val();

            var amountng = $("#amountngn");
            var rs = this.value;

           
                $.getJSON('/home/get_rate_usd', function (json) {
                 
                        if (type == "sell") {


                            amountng.val(rs * json[1].we_buy * json[0].we_buy).toFixed(2);


                            //label2.html("<b>You will pay " + ((0.10 * json.price) + json.price).toFixed(4) + " BTC </b>");
                        }
                        else {


                            amountng.val(rs * json[1].we_sell * json[0].we_sell).toFixed(2);
        ...
        $("#amountngn").on("input",function (e) {
            var type = $("#type").val();
            //var selectecurrency = $("#selectecurrency").val();

            var amountng = $("#amountbtc");
            var rs = this.value;
                ...
                $.getJSON('/home/get_rate_usd', function (json) {
                    if (type == "sell") {
                        ...
                        amountng.val((rs / (json[1].we_buy * json[0].we_buy)).toFixed(6));
                        ...
                        amountng.val((rs / (json[1].we_sell * json[0].we_sell)).toFixed(6));
        ...
        $('#selectecurrency').change(function () {
            $("#currency1").html(this.value);
        });
        ...
        $('#ecurrency_name2').change(function () {
            ...
            if (ecurrencyname != null || ecurrencyname != "0" || ecurrencyname != 0) {
                ...
                $.getJSON('/memberarea/loadamount?ecurrencyname=' + ecurrencyname, function (json) {
                    ...
                    label.text(json.ecurrency_name + " Account");
                    if (label3.val() != null || label3.val() != "" || label3.val() != 0) {
                        label2.val(json.we_sell * label3.val());
                    }
        ...
        $("#payable_in_Naira2").on("input", function (e) {
            ...
            $.getJSON('/memberarea/loadamount?ecurrencyname=NGN', function (json) {
                //alert(json.we_buy * amount);
                label2.val((amount / json.we_buy).toFixed(8));
            });
